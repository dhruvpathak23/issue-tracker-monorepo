...
def root(db: Session = Depends(get_db)):
    """Root endpoint with live database viewer"""
    ...
    users_html = ""
    for user in users:
        active_status = "✓ Active" if user.isActive else "✗ Inactive"
    ...
    html = ('<!DOCTYPE html><html lang="en"><head>'
            ...
            '<header><h1>🎯 Issue Tracker</h1><div class="version">Backend API v1.0.0</div><span class="status">✓ Running</span></header>'
            '<div class="refresh-note">🔄 This page refreshes every 5 seconds to show live database updates</div>'
            '<div class="stats">'
            ...
            f'{issues_display}'
            '</tbody></table></div>'
            '<footer><p>📱 Frontend: <a href="http://localhost:4200" style="color: #667eea; text-decoration: none;">http://localhost:4200</a></p>'
            '<p>Made with ❤️ | SQLite Database | FastAPI Backend</p></footer>'
            '</div></body></html>')
